...
package graduation.java.spark.ad;
...
/**
 * FileName: AdClickRealTimeStatSpark
 * Author:   hadoop
...
//				.set("spark.streaming.receiver.writeAheadLog.enable", "true");
        ...
        Logger.getLogger("19/04/19").setLevel(Level.ERROR);
		...
		//adRealTimeLogDStream.repartition(10);


        // 根据动态黑名单进行数据过滤
        JavaPairDStream<String, String> filteredAdRealTimeLogDStream =filterByBlacklist(adRealTimeLogDStream);

        filteredAdRealTimeLogDStream = filteredAdRealTimeLogDStream.persist(StorageLevel.MEMORY_AND_DISK());
        // 生成动态黑名单
...
//    }
    ...
    private static void generateDynamicBlacklist(
        ...
        dailyUserAdClickDStream.repartition(8);
...
//				}, 1000);
    ...
    private static JavaPairDStream<String, Long> calculateRealTimeStat(
        ...
        //mappedDStream = mappedDStream.cache();
                ...
                rdd.foreachPartition(new VoidFunction<Iterator<Tuple2<String,Long>>>() {
                    ...
                    public void call(Iterator<Tuple2<String, Long>> iterator)
                            ...
                            while(iterator.hasNext()) {
                                ...
                                //输出adStat
                                System.out.println(adStat.toString());
        ...
        JavaDStream<Row> rowsDStream = adRealTimeStatDStream.transform(
                ...
                new Function<JavaPairRDD<String,Long>, JavaRDD<Row>>() {
                    ...
                    public JavaRDD<Row> call(JavaPairRDD<String, Long> rdd)
                        ...
                        Dataset dailyAdClickCountByProvinceDs = sqlContext.createDataFrame(rowsRDD, schema);

                        // 将dailyAdClickCountByProvinceDF，注册成一张临时表
                        dailyAdClickCountByProvinceDs.registerTempTable("tmp_daily_ad_click_count_by_prov");

                        // 使用Spark SQL执行SQL语句，配合开窗函数，统计出各身份top3热门的广告
                        Dataset provinceTop3AdDs = sqlContext.sql(
                                "SELECT "
                                        + "date,"
                                        + "province,"
                                        + "ad_id,"
                        ...
                        //检查广告筛选
                        //provinceTop3AdDs.show();

                        return provinceTop3AdDs.javaRDD();
                    }
            ...
            public void call(JavaRDD<Row> rdd) throws Exception {
                ...
                rdd.foreachPartition(new VoidFunction<Iterator<Row>>() {
                    ...
                    public void call(Iterator<Row> iterator) throws Exception {
                        System.out.println("Row: "+iterator.hasNext());
                        if(iterator.hasNext()){
            ...
            public void call(JavaPairRDD<String, Long> rdd) throws Exception {
                ...
                rdd.foreachPartition(new VoidFunction<Iterator<Tuple2<String,Long>>>() {
                    ...
                    public void call(Iterator<Tuple2<String, Long>> iterator)
                        ...
                        System.out.println("Trend: "+ iterator.hasNext());
                        ...
                        if(flag){
                            ...
                            while(iterator.hasNext()) {
                                ...
                                //检查广告点击趋势
                                System.out.println(adClickTrend.toString());
